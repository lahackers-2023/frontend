...
const SignUp = () => {
  ...
  const onSubmit = async (event) => {
    ...
    fetch("http://localhost:8000/users", requestOptions)
      ...
      .then((result) =>
        navigate("/home", { state: { uid: JSON.parse(result)["uid"] } })
      )
  ...
  return (
    <div className="page">
      <h1 className="title">Sincerely,</h1>
      <form className="form" onSubmit={onSubmit}>
        <input
          id="fname"
          placeholder="First Name"
          value={fname}
          onChange={(newValue) => setFname(newValue.target.value)}
          required
        />
        <input
          id="lname"
          ...
          value={lname}
          onChange={(newValue) => setLname(newValue.target.value)}
          required
        />
